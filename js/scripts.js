...
$(document).ready(function() {
    ...
    function reposModal() {
        $(".foundry_modal").each(function() {
            var $this = $(this);
            var thisHeight = $this.height() + parseInt($this.css("paddingTop")) + parseInt($this.css("paddingBottom"));
            var thisWidth = $this.width() + parseInt($this.css("paddingLeft")) + parseInt($this.css("paddingRight"));
            console.log(thisHeight)
            var windowHeight = $(window).height();
            var windowWidth = $(window).width();
            $this.css({ "top": (windowHeight - thisHeight) / 2 })
            $this.css({ "left": (windowWidth - thisWidth) / 2 })
        })
    }
    $(window).on("resize", function() {
        reposModal()
    })
    setTimeout(reposModal,2000)
